...
describe('Specialized Transport Interception', function () {
    ...
    describe('Streaming', function () {
        it('modifies headers of a streaming request', function () {
            $promise = Http::client()
                ->withRequestInterceptor(fn (RequestInterface $r) => $r->withHeader('X-Stream-Test', 'active'))
        ...
        it('allows interceptResponse to modify a StreamingResponse', function () {
            $promise = Http::client()
                ->withResponseInterceptor(fn (ResponseInterface $res) => $res->withHeader('X-Stream-Processed', 'true'))
                ->stream(HttpBin::url('/stream/1'))
            ;
            $response = $promise->wait();
    ...
    describe('Downloads', function () {
        it('modifies request headers for a download', function () {
            ...
            $promise = Http::client()
                ->withRequestInterceptor(fn (RequestInterface $r) => $r->withHeader('X-Download-Token', 'abc-123'))
    ...
    describe('Uploads', function () {
        ...
        it('supports async interceptors before starting an upload', function () {
            ...
            $promise = Http::client()
                ->withRequestInterceptor(fn ($r) => delay(0.1)->then(fn () => $r->withHeader('X-Waited', 'true')))
